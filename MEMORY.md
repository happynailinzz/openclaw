# MEMORY.md

## 长期偏好与工作约束（2026-02-22）

- 用户偏好中文、直给、少废话风格。
- 用户明确要求：默认 web search 使用 Tavily，不推荐/不使用 Brave（除非用户明确指定）。
- 用户将心跳检查重点固定为：
  1) AI 热点选题
  2) 政企数字化/智能化热点选题
  3) 海外中文新闻（chinanewsx）
  4) 进行中任务进展
- 心跳策略：每 1 小时，活跃时段 08:30-23:00，没新变化则 `HEARTBEAT_OK`。
- 用户内容方向近期聚焦：政企数字化与数据治理主题，强调“去 AI 味”文风与公众号可发布成稿。
- 用户执行偏好：复杂系统改造类任务先“固化 SOP”，后续按需触发执行（先规范、再实战）。
- 用户新增写作工作流约束（2026-02-23）：在「公众号选题、内容策划、写作与改稿」相关任务中，信息源优先级固定为：
  1) `/opt/openclaw/knowledge/secendME`（个人 Obsidian 知识库）
  2) `MEMORY.md` 与历史会话记忆
  3) 外部互联网资料
  输出时需标注：哪些来自知识库（含笔记名/文件）、哪些是基于其延伸、哪些是纯外部信息；若库内材料不足再外搜，且外部风格不得盖过用户既有风格。
- 用户事业认知基线（2026-02-23，基于其官网寰曜数能）：其主要工作载体为“寰曜数能（huanyao.yuweixun.site）”，定位为中原区域（郑州锚点）面向 G/B 端的政企数字化转型与智能升级综合服务商，不是泛 IT 外包。
- 其事业的核心业务版图固定为六大方向：智能化与数字化转型、算力与数据中心、AI 应用落地、信创国产化、数据标注/数据治理、源网荷储信息化（能源数字化）。
- 其差异化能力框架："四位一体"（咨询规划、系统集成、产品销售、项目投融资）端到端服务；后续在分析其业务、提供选题/咨询建议时，应优先围绕“闭环交付能力”而非单点技术能力展开。
- 其优先行业航道（内容与咨询重点）应默认聚焦：1) 能源数字化/智慧电力/源网荷储；2) 国企与政府数字化、信创替代；3) 智慧教育与产教融合。
- 关于用户事业的默认表达口径：本质是在国家数字化政策与行业真实业务场景之间做连接，用“咨询+集成+生态+资金”组合拳，将客户目标转化为可交付、可验收、可持续的经营结果。
- 后续内容创作与信息搜集的底层逻辑：优先寻找“政策变化 → 行业场景 → 项目机会 → 落地路径 → 量化结果”的链路信息；避免仅做趋势转述或技术名词堆砌。
- 用户新增流程要求（2026-02-24）：OpenClaw 产出的公众号「选题」与「文章草稿」默认入库 Notion 的“文章选题草稿箱”。
- 用户新增 Get笔记规则（2026-03-13）：以后用户在对话中直接贴链接时，若没有特别说明，默认通过 getnote skill 将链接导入 Get笔记；同时将 Get笔记 视为记忆系统的外部笔记落点之一。
- 用户近期监控重点更新（2026-03-08）：优先关注“两会”及“河南在两会上的相关领域”新闻与政策动态，尤其是政企数字化、算力与数据中心、AI应用落地、能源数字化（源网荷储/智慧电力）、教育数字化/产教融合。
- 用户新增关注方向（2026-04-12）：后续选题监控与公众号策划除既有六大业务版图外，额外纳入两条重点观察线：
  1) AI 漫剧 / AI 生剧 / AIGC 影视内容工业化（重点看制作工作流、降本提效、商业模式、平台分发、版权与供给侧变化）；
  2) 源网荷储扩展到绿色储能、风电、光伏发电（重点看项目机会、投资逻辑、政策抓手、园区/电力/工商业场景落地与交付链条）。
- 用户进一步校正要求（2026-04-12）：两条新增方向分开处理：
  1) 源网荷储 / 绿色储能 / 风电 / 光伏发电：保持当前河南口径，后续优先聚焦河南省相关厅局、地方政府、园区平台发布的政策、产业支持、补贴、申报通知、试点名单、项目清单与资金口径；选题上优先写“谁发文、谁给钱、谁能报、谁落地”。
  2) AI 漫剧 / AI 生剧 / AIGC 影视内容工业化：回退到上一版，不强行只盯河南厅局口径，继续优先关注内容工业化、制作工作流、降本提效、商业模式、平台分发、版权与供给侧变化等更泛产业线索。
- 用户执行偏好更新（2026-02-27 13:40）：公众号相关流程可默认一次性跑完，无需每一步确认；仅在高风险动作、不可逆操作或关键信息缺失时中断确认。
- 用户新增固定作者要求（2026-02-27）：后续公众号文章作者统一使用“余炜勋”。
- 代理能力进化规则已固化（2026-03-01）：兼容版四条规则见 `/root/.openclaw/workspace/memory/agent-evolution-rules.md`，用于任务内能力沉淀、轻量复盘、反劣化锁与能力树管理。
- 记忆整合方案已固化（2026-03-01）：采用「本地热记忆 + Memos 冷记忆」分层路由，默认检索只注入 `local_hits + memos_hits.hits` 精简片段以节省 token；配置见 `config/memory-integration.json`，流程见 `scripts/memory-cycle.sh` 与 `docs/memory-integration-deploy.md`。
- 记忆系统必须并行纳入 Notion 文章收集与选题体系（2026-03-01）：公众号相关任务检索路由需将 Notion「文章选题草稿箱」作为优先来源之一，不可仅依赖本地记忆与 Memos。
- 用户新增工作结论（2026-03-09）：公众号正式发布主链应以 `scripts/publish.sh` 为准，但快反文章默认走 `--fast` 模式；`--fast` 必须直接跳过自动生图，优先使用 `--cover`、现成封面或默认占位图，避免 SmartAPI 图片生成阻塞整条发布链。
- 用户新增内容样式要求（2026-03-09）：公众号文章排版不要泛卡片/杂志风；更偏好参考《工信部在“部长通道”释放明确信号：2026年，AI进入制造业将从概念推进转向场景落地》的公众号版式（居中下划线主标题、蓝色副标题/作者、蓝底白字二级标题、左侧蓝线引用块）；后续文章默认沿用该风格并优先直接输出 refined wechat HTML，且必须避免出现单独的“。”或零碎残句单独占一行。
- 用户新增配图风格要求（2026-03-09）：《从3个信号看，河南电力数字化的项目窗口已经打开》及同类稿件的配图默认不走“杂志风”，改为“电力系统信息图 / 产业决策可视化 / 监管科技感”；强调深蓝背景、节点网络、能量流、电网拓扑、平台界面与基础设施系统感。
- 用户新增运维结论（2026-03-09）：Cron 应做分层治理；测试残留任务应及时删除/停用，真正需要看到的任务统一明确投递到 Telegram `1511915498`，避免 `delivery.mode=none` 或 `channel=last` 导致“任务跑了但用户没收到”的假完成。
- 已完成 cron 第一轮与第二轮整改（2026-03-09）：删除 `test:lianghui-henan-industries:now`，停用 `test:lianghui-henan-industries:next-minute`，并将 `digest:overseas-cn-news:0800`、`digest:overseas-cn-news:1400`、`monitor:github-updates:0900`、`digest:equity-tracks:1600`、`report:ai-infra-capital-weekly:mon1005` 统一改为明确投递到 Telegram `1511915498`。
- 用户补充校正（2026-03-10）：教育数字化 / 产教融合 / 职业教育 / 数字人才培养，不是孤立新增方向，而应严格挂回其既有的寰曜数能业务版图与优先行业航道中理解；后续相关内容与项目判断应继续服从“政企数字化转型 + 智能升级”总框架，以及既有三条优先航道中的“智慧教育与产教融合”主线。写法上应重点落在“人才供给体系重构、校企协同、实训能力、能力评估与区域人才供给”，而不是泛泛写“AI课堂”。
- 运维修复结论补充（2026-03-10）：`message send` 的 poll 误判在 OpenClaw 2026.3.8 更新后会因 bundle 文件名变化而复发；需要按当前版本实际 `dist/*` bundle 全量补丁并重启后验证，文本与附件发送链路都要单独回归。
- 服务器搜索规则补充（2026-03-10）：在这台服务器上，后续搜索任务默认主路由切到 Tavily；执行顺序为“本地知识库 / 记忆 -> Tavily skill 脚本 -> web_fetch / browser 定点补充”。Brave 不再作为默认搜索链路，除非用户明确指定 Brave。
